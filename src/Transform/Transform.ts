...
import { M00, M11, M22, N0, N1, N2 } from "../definitions";
import type { TransformJSON } from "../types";
...
export class Transform {
  ...
  public set({
    ...
    if (rotationOriginAxis) {
      this.setRotationOriginAxis(rotationOriginAxis);
    }
    if (rotationOriginScale) {
      this.setRotationOriginScale(rotationOriginScale);
    }
    if (origin) {
      this.setOrigin(origin);
    }
    if (rotationCenterAxis) {
      this.setRotationCenterAxis(rotationCenterAxis);
    }
    if (rotationCenterScale) {
      this.setRotationCenterScale(rotationCenterScale);
    }
    if (scale) {
      this.setScale(scale);
    }
